export default class UserIssueProgress {
    ...
    constructor(userIssueProgress?: number) {
        if (userIssueProgress && !UserIssueProgress.USER_ISSUE_PROGRESS_MATCHER.test(String(userIssueProgress))) {
            throw new Error('Do not match UserIssueProgress FORMAT')
        }
        this.userIssueProgress = userIssueProgress ?? UserIssueProgress.PROGRESS_NONE;
    }

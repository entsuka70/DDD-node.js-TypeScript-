...
export default class PairRepository implements PairRepositoryInterface {
  ...
  public async findMinUser(pair: Pair): Promise<Pair> {
    ...
    // ユーザー数2名のペア抽出
    const minUserPairs = pairs.filter(
      (pair) => pair._count?.user == Pair.MIN_ACCEPTABLE_PAIR_USER
    );
    ...
    const isExistsUserArray = minUserPairs.map((mP) =>
      mP.user.map((u) => pair.isExistUser(u.id))
    );

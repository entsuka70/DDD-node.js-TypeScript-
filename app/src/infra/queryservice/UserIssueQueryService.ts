...
import UserIssueQueryServiceInterface from 'src/app/application/userIssue/UserIssueQueryServiceInterface';
import UserIssueListDto from 'src/app/application/userIssue/UserIssueListDto';
import UserListDto from 'src/app/application/userIssue/UserListDto';
import UserIssueGetCommand from 'src/app/application/userIssue/UserIssueGetCommand';
  ...
  implements UserIssueQueryServiceInterface
{
  ...
  public async find(command: UserIssueGetCommand): Promise<UserIssueListDto[]> {
    ...
    const userIssueLists = await this.prisma.userIssue.findMany({
      ...
      where: searchRule,
      skip:
        command.list_no && command.list
          ? Number(command.list) * Number(command.list_no)
          : undefined,
      take: command.list
        ? Number(command.list)
        : UserIssueQueryService.DEFAULT_PAGINATION,
      orderBy: {
        issue_id: 'asc',
      },
    });
  ...
  public setQueryRule(command: UserIssueGetCommand): SearchRule {
    ...
    command.progress
      ? (searchRule.progress = Number(command.progress))
      : searchRule;
    command.issue_no
      ? (searchRule.issue.issue_no = Number(command.issue_no))
      : searchRule;
    command.issue_name
      ? (searchRule.issue.issue_name = command.issue_name)
      : searchRule;
    command.issue_group
      ? (searchRule.issue.issue_group = Number(command.issue_group))
      : searchRule;
    command.user_name

  ...
  public async update(command: PairCreateCommand) {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          `UserId does not exist. You can not register ${command.user_ids}`
        );
      }
      ...
      if (pairRebuild.getUserIds().length == Pair.MAX_PAIR_USER) {
        ...
        if (!shouldJoinPairs || shouldJoinPairs.length <= 2) {
          throw new Error(
            `The pair that user can join is too few. ${shouldJoinPairs}`
          );
        }
      ...
      if (
        ...
        const pairRebuildforException = await this.pairFactory.update(
          command,
          pair
        ); // 例外検出用にpairRebuildと同じオブジェクトを形成
        ...
        if (pair.getUserIds().length == Pair.MIN_PAIR_USER) {
          const pairMinUserForLeft = await this.pairRepository.findMinUser(
            pair
          );
